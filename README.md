...
/path/to/hermes-agent/node_modules/.bin/tsc -p tests/tsconfig.desktop-plugin.json
# FastAPI bridge test needs Hermes venv (unset PYTHONPATH so the repo's tests/ module wins):
env -u PYTHONPATH /path/to/hermes-agent/venv/bin/python -m unittest tests.test_plugin_api -v
```
